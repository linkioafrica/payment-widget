      ...
      <div className="flex w-full flex-col relative">
        ...
        {isActive && (
          <div className="w-full absolute -bottom-1 z-10">
            ...
            ></div>
            <div className="w-full white max-h-[120px] flex flex-col translate-y-full border-[0.8px] border-[#E2E3E7] bg-[#F3F4F6] rounded-lg py-1 overflow-auto shadow-lg dark:bg-[#101113] dark:border-[#242425]">
              {dropDownData
                .filter((data: any) => {
                ...
                .map((data: any, index: number) => (
                  <span
                    key={index}
                    className="py-3 px-3 hover:bg-[#E2E3E7] select-none  cursor-pointer dark:text-[#F9F9F9] text-black dark:hover:bg-[#282828]"
                    onClick={() => {
    ...
    return (
      <div className="flex w-full flex-col relative">
        ...
        >
          <input
            type="text"
            className="w-full outline-none text-xs px-3 py-2 rounded-lg bg-[#F3F3F3] dark:bg-[#141415] dark:text-white"
            placeholder={placeholder}
            value={searchValue}
            onChange={(change) => setSearchValue(change.target.value)}
            // Hide dropdown on blur
            ...
            <div className="w-full white max-h-[120px] flex flex-col translate-y-full border-[0.8px] border-[#E2E3E7] bg-[#F3F4F6] rounded-lg py-1 overflow-auto shadow-lg dark:bg-[#101113] dark:border-[#242425]">
              {dropDownData
                ...
                .map((data: any, index: number) => (
                  <span
                    key={index}
                    className="py-2 px-3 hover:bg-[#E2E3E7] select-none text-xs cursor-pointer dark:text-[#F9F9F9] text-black dark:hover:bg-[#282828]"
                    onClick={() => {
                      setSearchValue(data[dropDownDataKey]);
                      setIsActive(false);
                    }}
